...
def load_and_preprocess_data(positive_file, negative_file, pseudo_pos_file=None, pseudo_neg_file=None, add_pseudo=False):
    ...
    if add_pseudo:
        ...
        balanced_train_data = pd.concat([balanced_train_data, pseudo_pos_samples_upsampled, pseudo_neg_samples], ignore_index=True)
    ...
    def tokenize_function(examples):
        tokens1 = tokenizer1(examples["sequence"], padding="max_length", max_length=MAX_LEN_1)
        tokens2 = tokenizer2(examples["sequence"], padding="max_length", max_length=MAX_LEN_2)
...
training_args = TrainingArguments(
    output_dir=SAVED_MODEL_DIR_3D8H256D_P4,
    ...
    logging_dir=os.path.join(SAVED_MODEL_DIR_3D8H256D_P4, 'logs'),  # Directory for storing logs
...
trainer.save_model(SAVED_MODEL_DIR_3D8H256D_P4)
...
# 保存评估结果
with open(os.path.join(SAVED_MODEL_DIR_3D8H256D_P4, "best_model_eval_metrics.txt"), "w") as f:
